...
class MyOneHotPartialObsWrapper(gym.core.ObservationWrapper):
    """
    Wrapper to get a one-hot encoding of a partially observable
    agent view as observation.
    """
    OBJECT_TO_CODE_NO_AGENT = {
        2: 0,  # 'wall': 0,
        11: 1,  # 'dirt': 1,
        12: 2,  # 'button': 2,
        13: 3,  # 'mess': 3,
    ...
    shape = (len(OBJECT_TO_CODE_NO_AGENT), 7, 7)
    def cc__init__(self, env, tile_size=8):
        super().__init__(env)
        ...
        self.observation_space.spaces["image"] = spaces.Box(
            low=0,
            high=255,
            shape=MyOneHotPartialObsWrapper.shape,
            dtype='uint8'
        )
